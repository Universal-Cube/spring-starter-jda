...
def create_tag(version):
    ...
    try:
        subprocess.run(['git', 'tag', tag_name], capture_output=True, text=True, check=True)
...
def push_tag(tag_name):
    try:
        subprocess.run(['git', 'config', 'user.name', 'github-actions[bot]'], capture_output=True, text=True, check=True)
        subprocess.run(['git', 'config', 'user.email', 'github-actions[bot]@users.noreply.github.com'], capture_output=True, text=True, check=True)
